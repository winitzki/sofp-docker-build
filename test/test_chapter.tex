...
Interpreter is parameterized by a \textsf{``}value extractor\textsf{''} {\footnotesize{}$\text{Ex}^{F}\triangleq\forall A.\left(F^{A}\rightarrow A\right)$}{\footnotesize\par}
...
Suppose we have a value extractor of type $\text{Ex}^{F}\triangleq\forall A.\left(F^{A}\rightarrow\text{Err}+A\right)$
...
{*}{*}{*}\footnote{\textsf{``}A function \lstinline!launch()! that launches real-world missiles
...
made to work for 2 or 3-dimensional vectors
...
\[
T^{A}\triangleq A+(P\rightarrow T^{A})+Q\times T^{A}\quad,
\]
where $P$ and $Q$ are fixed types. (The monad $T$ represents computations
that may consume an input value of type $P$ or produce an output
value of type $Q$.) Show that $T$ is a lawful monad by reducing
$T$ to a free monad construction.
...
(a) Implement a monadic DSL with operations put: A => Unit and get:
Unit => A; run example programs written in that DSL. 

(b) Implement a monadic DSL with operations put: A => Unit, get: Unit
=> Option{[}A{]}, and clear: Unit => Unit, with the functionality
that a value may be deleted (and then get should return None). Run
example programs.
...
Show: $\forall X.X=\bbnum 0$; $\exists Z.Z\cong\bbnum 1$; $\exists Z.Z\times A\cong A$;
$\forall A.\left(A\times A\times A\rightarrow A\right)\cong\bbnum 1+\bbnum 1+\bbnum 1$.
...
\]
It was shown in Statement~\ref{subsec:Statement-monad-construction-4}
...
The free monad $\text{Free}^{F}$ is natural in the functor $F$:
...
type constructor such that the resulting type has the required monad
operations. If $F$ is already itself a monad, constructing the free
monad on $F$ seems to be unnecessary. Indeed, a value of type $\text{Free}^{F,A}$
...
Assume that $F$ is a monad, and denote $T\triangleq\text{Free}^{F}$
for brevity.
...
\textbf{(a)} To verify the identity law:
\begin{align*}
...
\textbf{(b)} The injection map $q(f)\triangleq\bbnum 0+f\triangleright(x\rightarrow x+\bbnum 0)^{\uparrow F}$
...
$A$ can be generalized to involve an arbitrary functor $F$. The
Church encoding of a simple type is then regarded as a particular
case of the Yoneda identity where the functor $F$ is chosen as the
identity functor.
...
used as an argument of \lstinline!fromC!, is always a natural transformation.
To verify the naturality law~(\ref{eq:assumed-naturality-of-argument-q}):
\begin{align*}
...
If $A$ is any fixed type and $H^{\bullet}$ is any contrafunctor,
the type $H^{A}$ is equivalent to the function type $\forall X.\,(X\rightarrow A)\rightarrow H^{X}$,
where the functions of that type are additionally required to be natural
...
used as an argument of \lstinline!fromC!, is always a natural transformation.
To verify the naturality law~(\ref{eq:assumed-naturality-of-argument-q-contravariant}):
\begin{align*}
...
a higher-order functor, i.e., $S$ depends covariantly on an arbitrary
type constructor $F^{\bullet}$. (An example of such $S$ is $S^{F}\triangleq F^{\text{Int}}\times F^{\text{String}}$.)
...
(while its argument, of type $P^{\bullet}\leadsto F^{\bullet}$, is
not necessarily a natural transformation since $P$ and $F$ are not
required to be functors). The corresponding naturality law for functions
$\sigma$ of type $\forall F^{\bullet}.\,(P^{\bullet}\leadsto F^{\bullet})\rightarrow S^{F}$
...
A recursive type is usually defined as a fixpoint of a type equation\index{recursive type equation}
...
now consider the question of whether there can be several fixpoint
$T$.

If a fixpoint equation has several \emph{inequivalent} solutions $T_{1}$,
$T_{2}$, etc., it means that each $T_{i}$ separately satisfies the
fixpoint equation. An example is the fixpoint equation for the \textsf{``}lazy
list\textsf{''}:
\begin{equation}
...
that converts a value of type $L^{A}$ into an ordinary (eager) list
of values of type $A$. The function \lstinline!toList! keeps recursively
requesting new elements of the on-call list and accumulates the resulting
values of type $A$:
\[
...
number of values and thus are equivalent to ordinary (eager) lists.
To make this equivalence formal, we may define isomorphism functions
$\text{toList}:L_{\text{fin}}^{A}\rightarrow\text{List}^{A}$ and
$\text{fromList}:\text{List}^{A}\rightarrow L_{\text{fin}}^{A}$. 
...
and then we could compose $q$ with \lstinline!toList! to obtain
a function $\text{toList}:L_{\text{inf}}^{A}\rightarrow\text{List}^{A}$
...
as, $\text{wrap}\bef\text{unwrap}$) of these isomorphisms will act
...
For instance, $L^{A}$ is larger than either of $L_{\text{fin}}^{A}$
and $L_{\text{inf}}^{A}$. To show this formally: {*}{*}{*}
...
The main use case for Church encoding is for recursive types.

Consider a recursive type $T$ defined by a fixpoint equation $T\triangleq F^{T}$
with a given structure functor $F$. We could write the Church encoding
for $T$ as $\forall X.\,(T\rightarrow X)\rightarrow X$ or as $\forall X.\,(F^{T}\rightarrow X)\rightarrow X$,
but this would bring no advantages. It turns out that another, more
useful Church encoding for $T$ is given by
\begin{equation}
...
where the type $A$ does not depend on the quantified type $X$. 
...
\]
Then the type $T$ is a solution of the fixpoint equation $T\cong F^{T}$.

\subparagraph{Proof}

We need to implement the isomorphisms
\[
\text{wrap}:T\rightarrow F^{T}\quad,\quad\quad\text{unwrap}:F^{T}\rightarrow T\quad.
\]
Begin by implementing \lstinline!unwrap!. The required type signature
is
\[
\text{unwrap}:F^{\forall X.\,(F^{X}\rightarrow X)\rightarrow X}\rightarrow\forall Y.\,(F^{Y}\rightarrow Y)\rightarrow Y\quad.
\]
We will derive the code of \lstinline!unwrap! via typed holes:
\[
\text{unwrap}=f^{:F^{\forall X.\,(F^{X}\rightarrow X)\rightarrow X}}\rightarrow\forall Y.\,u^{:F^{Y}\rightarrow Y}\rightarrow\text{???}^{:Y}\quad.
\]
The only way of computing a value of type $Y$ is to apply $u$ to
an argument of type $F^{Y}$:
\[
\text{unwrap}=f^{:F^{\forall X.\,(F^{X}\rightarrow X)\rightarrow X}}\rightarrow\forall Y.\,u^{:F^{Y}\rightarrow Y}\rightarrow\big(\text{???}^{:F^{Y}}\big)\triangleright u\quad.
\]
The functor $F$ is arbitrary, so the only way of computing a value
of type $F^{Y}$ is to use the given value $f$. Since the type parameter
$X$ is universally quantified inside $f$, we may set $X$ to any
type as needed. So, we set $X=Y$ under $F^{\forall X.\,(...)}$;
the result, denoted for brevity by $f^{Y^{\uparrow F}}$, has type
\[
f^{Y^{\uparrow F}}:F^{(F^{Y}\rightarrow Y)\rightarrow Y}\quad.
\]
We can now use this value together with $u^{:F^{Y}\rightarrow Y}$
to fill the last type hole:
\[
\text{unwrap}\triangleq f^{:F^{\forall X.\,(F^{X}\rightarrow X)\rightarrow X}}\rightarrow\forall Y.\,u^{:F^{Y}\rightarrow Y}\rightarrow f^{Y^{\uparrow F}}\triangleright\big(p^{:(F^{Y}\rightarrow Y)\rightarrow Y}\rightarrow p(u)\big)^{\uparrow F}\triangleright u\quad.
\]

We turn to implementing wrap. Write the type signature as
\[
\text{wrap}:\big(\forall Y.\,(F^{Y}\rightarrow Y)\rightarrow Y\big)\rightarrow F^{\forall X.\,(F^{X}\rightarrow X)\rightarrow X\big)}\quad.
\]
 To implement \lstinline!wrap!, we need to compute a value of type
$F^{T}=F^{\forall X.\,(F^{X}\rightarrow X)\rightarrow X}$:
\[
\text{wrap}=t^{:\forall Y.\,(F^{Y}\rightarrow Y)\rightarrow Y}\rightarrow\text{???}^{:F^{T}}\quad.
\]
The only way of getting a value of type $F^{T}$ is to use $t^{Y}$
while setting the type parameter $Y$ to $F^{T}$:
\[
\text{wrap}=t^{:\forall Y.\,(F^{Y}\rightarrow Y)\rightarrow Y}\rightarrow t^{F^{T}}\big(\text{???}^{:F^{F^{T}}\rightarrow F^{T}}\big)\quad.
\]
We can fill the typed hole $\text{???}^{:F^{F^{T}}\rightarrow F^{T}}$
by lifting \lstinline!unwrap! to the functor $F$:
\[
\text{wrap}\triangleq t^{:\forall Y.\,(F^{Y}\rightarrow Y)\rightarrow Y}\rightarrow t^{F^{T}}\big(\text{unwrap}^{\uparrow F}\big)\quad.
\]

It remains to show that \lstinline!wrap! and \lstinline!unwrap!
are inverses to each other:
\begin{align*}
{\color{greenunder}\text{expect to equal }t:}\quad & t^{:T}\triangleright\text{wrap}\bef\text{unwrap}=t^{F^{T}}(\text{unwrap}^{\uparrow F})\triangleright\text{unwrap}\\
 & =\forall Y.\,u^{:F^{Y}\rightarrow Y}\rightarrow\big(t^{F^{T}}(\text{unwrap}^{\uparrow F})\big)^{Y^{\uparrow F}}\triangleright\big(p^{:(F^{Y}\rightarrow Y)\rightarrow Y}\rightarrow p(u)\big)^{\uparrow F}\triangleright u\quad.
\end{align*}
The resulting function will be equal to $t$ if we show that it gives
the same values when applied to an arbitrary argument $u^{:F^{Y}\rightarrow Y}$
(with an arbitrarily set type $Y$):
\[
t^{Y}(u^{:F^{Y}\rightarrow Y})\overset{?}{=}\big(t^{F^{T}}(\text{unwrap}^{\uparrow F})\big)^{Y^{\uparrow F}}\triangleright\big(p^{:(F^{Y}\rightarrow Y)\rightarrow Y}\rightarrow p(u)\big)^{\uparrow F}\triangleright u\quad.
\]
{*}{*}{*}

This proves $\text{wrap}\bef\text{unwrap}=\text{id}$, which is one
direction of the isomorphism between $T$ and $F^{T}$. 

To verify the opposite direction of the isomorphism:
\begin{align*}
{\color{greenunder}\text{expect to equal }f:}\quad & f^{:F^{T}}\triangleright\text{unwrap}\bef\text{wrap}=\big(\forall Y.\,u^{:F^{Y}\rightarrow Y}\rightarrow f^{Y^{\uparrow F}}\triangleright\big(p^{:(F^{Y}\rightarrow Y)\rightarrow Y}\rightarrow p(u)\big)^{\uparrow F}\triangleright u\big)\triangleright\text{wrap}\\
 & =f^{(F^{T})^{\uparrow F}}\triangleright\big(p^{:(F^{F^{T}}\rightarrow F^{T})\rightarrow F^{T}}\rightarrow p(\text{unwrap}^{\uparrow F})\big)^{\uparrow F}\triangleright(\text{unwrap}^{\uparrow F})\quad.
\end{align*}
The last value will be equal to $f$ if {*}{*}{*}
...
For any functor $F$ and contrafunctor $H$, the following identities
hold:
\begin{align*}
{\color{greenunder}\text{covariant co-Yoneda identity}:}\quad & \exists R.\,F^{R}\times\left(R\rightarrow A\right)\cong F^{A}\quad,\\
{\color{greenunder}\text{contravariant co-Yoneda identity}:}\quad & \exists R.\,H^{R}\times\left(A\rightarrow R\right)\cong H^{A}\quad.
\end{align*}
...
and $\forall B.(\forall A.P^{A,A}\rightarrow B)\rightarrow B$ are
equivalent when restricted to fully parametric implementations.

\textbf{(b)} Without the outer quantifier ($\forall B$), this property
does not hold: the types $(\exists A.P^{A,A})\rightarrow B$ and $\forall A.(P^{A,A}\rightarrow B)$
are \emph{not} equivalent in general.
